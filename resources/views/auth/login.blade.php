...
<div class="flex min-h-[80vh] items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
        ...
        <div class="mb-8 text-center">
            ...
            <h1 class="text-2xl font-black">{{ __('messages.login') }}</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">أهلاً بعودتك إلى {{ __('messages.app_name') }}</p>
        </div>
            ...
            <form method="POST" action="{{ route('login.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus
                           class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-cyan-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-white/10 dark:bg-white/5 dark:focus:bg-white/10 @error('email') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="password" name="password" required
                           class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-cyan-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-white/10 dark:bg-white/5 dark:focus:bg-white/10 @error('password') border-red-400 @enderror">
                    @error('password')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center justify-between">
                    <label class="flex cursor-pointer items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                        <input type="checkbox" name="remember" class="rounded accent-cyan-500">
                        تذكرني
                    </label>
                </div>
        ...
        <p class="mt-5 text-center text-sm text-slate-500 dark:text-slate-400">
            ليس لديك حساب؟
            <a href="{{ route('register') }}" class="font-bold text-cyan-600 hover:underline dark:text-cyan-400">
